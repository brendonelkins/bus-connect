...
test("confirm signal units and description", async ({ page }) => {
  ...
  const verifySearchResults = async (searchTerm: string, expectedText: string) => {
  ...
  await verifySearchResults('ActlMxAvlblRetPercentTorq', 'This is the maximum amount of torque that the retarder can immediately deliver. It is the same as the maximum torque')
  await verifySearchResults('ABSFullyOperational', 'Signal which indicates whether an ABS system is fully operational or whether its functionality is reduced by a defect or by')
  await verifySearchResults('ExhaustSystemHighTempLampCmd', 'Command to control the exhaust system high temperature lamp. This lamp indicates that the exhaust system')
});
